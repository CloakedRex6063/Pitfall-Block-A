...
public:
	...
	void Print(const char* s, int x1, int y1, uint c, int2 scale);
	void Line(float x1, float y1, float x2, float y2, uint c) const;
	void Clear( uint c );
	void ThickLine(float x1, float y1, float x2, float y2, float thickness, uint c) const;
	...
	void Bar(int2 a1, int2 b1, uint c);
	// attributes
	uint* pixels = 0;
