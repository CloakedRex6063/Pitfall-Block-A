...
{
	// Calculate the number of bytes to set using the size of the pixels array
	size_t numBytes = width * height * sizeof(uint);

	// Use memset to set all bytes to the value of c
	memset(pixels, c, numBytes);
}
...
void Surface::Bar(int2 a1, int2 b1, uint c)
{
	// clipping
	if (a1.x < 0) a1.x = 0;
	if (b1.x >= width) b1.x = width - 1;
	if (a1.y < 0) a1.y = 0;
	if (b1.y >= height) b1.y = width - 1;
	// draw clipped bar
	uint* a = a1.x + a1.y * width + pixels;
	for (int y = a1.y; y <= b1.y; y++)
	{
		for (int x = 0; x <= (b1.x - a1.x); x++) a[x] = c;
		a += width;
	}
}

// Modified with help of Quinten Y1
void Surface::Print( const char* s, int x1, int y1, uint c , int2 scale)
{
	if (!fontInitialized)
	{
	...
	for (int i = 0; i < (int)(strlen( s )); i++, t += 6 * scale.x)
	{
		int pos;
		...
		for (int v = 0; v < 5 * scale.x; v+=scale.x, u++, a += width * scale.y) {
			for (int h = 0; h < 5 * scale.x; h+=scale.x) {
				if (*u++ == 'o') {
					for (int x = 0; x < scale.y; x++) {
						for (int y = 0; y < scale.x; y++) {
							*(a + h + y + width * x) = c;
						}
					}
				}
...
void Surface::Line( float x1, float y1, float x2, float y2, uint c ) const
{
...
void Surface::ThickLine(float x1, float y1, float x2, float y2, float thickness, uint c) const
{
