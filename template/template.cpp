...
void KeyEventCallback( GLFWwindow*, int key, int, int , int )
{
...
void main()
{
	...
	app->Init();
	Shader* shader;
	if (dynamic_cast<Game*>(app)->settings.bUseCRT)
	{
	...
	{
		if (dynamic_cast<Game*>(app)->settings.bUseFXAA)
		{
			// fxaa shader
		shader = new Shader(
		"#version 330\nin vec4 p;\nin vec2 t;out vec2 uv;void main(){uv=t;gl_Position=p;}",
	...
	{
		deltaTime = min( .5f, 1.f * timer.elapsed() );
		fixedDeltaTime += deltaTime;
		while (fixedDeltaTime > FIXED_DELTA_TIME)
		{
			app->PhysicsTick();
